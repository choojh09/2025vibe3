...
# 상위 지역(시·도) 추출 함수: 특별자치시 포함
# '세종특별자치시' 같은 명칭도 하나의 행정구역으로 인식
...
if page == "지역별":
    ...
    fig = px.line(
        df_line, x="연령", y=sel_provinces, markers=True,
        labels={"value":"인구수","연령":""}
    )
    ...
    fig = px.bar(
        df_gen, x="연령", y=["남자","여자"], barmode="group",
    ...
    fig = px.line(
        df_grp, x="연령대", y="total", markers=True,
