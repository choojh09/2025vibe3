...
# ---- 5‑4. 미래 전망 + 주관적 평가 ----
    ...
    fig = px.bar(df_future.sort_values("EDI_future", ascending=False),
                 ...
                 labels={"EDI_future":f"EDI(+{g_ppi}%·{g_pph}%/y, {yrs}y)",
                         "region":"시·도"}, height=600)
    st.plotly_chart(fig, use_container_width=True)

    # 주관적 평가: 상·중·하
    rank = df_future.sort_values("EDI_future", ascending=False).reset_index(drop=True)
    n = len(rank); top, mid = int(np.ceil(n*0.35)), int(np.floor(n*0.65))
    rank["grade"] = ["상위"]*n
    rank.loc[top:mid-1,"grade"]="중위"
    rank.loc[mid:,   "grade"]="하위"

    st.markdown("### 📋 등급 & 수치")
    st.dataframe(rank[["region","EDI","EDI_future","grade"]],
                 use_container_width=True)

    st.markdown("### ✍️ 주관적 해석")
    for _, r in rank.iterrows():
        if r.grade=="상위":
            st.markdown(f"- **{r.region}**: 🌟 미래 EDI도 **우수**할 전망.")
        elif r.grade=="중위":
            st.markdown(f"- **{r.region}**: ➡️ **보통** — 정책·투자 여부에 따라 달라질 구간.")
        else:
            st.markdown(f"- **{r.region}**: ⚠️ **취약** — 인구·소비둔화로 개선 노력 필요.")
