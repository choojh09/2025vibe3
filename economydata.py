...
# 0. 기본 설정
# ─────────────────────────────
st.set_page_config(page_title="지역별 경제지표 대시보드", layout="wide")
...
if not (pop_file and ppi_file):
    st.info("👈 두 CSV를 모두 업로드하면 대시보드가 표시됩니다.")
    st.stop()

# ─────────────────────────────
# 2. 범용 CSV 로더
# ─────────────────────────────
def robust_read(upload):
    raw = upload.read()
...
def normalize_region(name: str) -> str:
    """
    공백·괄호·숫자 제거 후 시/도를 표준화하고
    결과를 공백 없이 붙여서 반환.
    예) '서울 특별시 노원구 (123)' → '서울특별시노원구'
    """
    raw = re.sub(r"\\([^)]*\\)", "", str(name))   # 괄호 제거
    raw = re.sub(r"\\d+", "", raw)               # 숫자 제거
    raw_no_space = raw.replace(" ", "")
    for k, v in CANON.items():
        k0, v0 = k.replace(" ", ""), v.replace(" ", "")
        if raw_no_space.startswith(k0):
            return v0 + raw_no_space[len(k0):]
        if raw_no_space.startswith(v0):
            return v0 + raw_no_space[len(v0):]
    return raw_no_space  # 매칭 실패 시 그대로(공백·숫자·괄호 제거된 형태)
...
pop_df[reg_pop_col] = pop_df[reg_pop_col].apply(normalize_region)
ppi_df[reg_ppi_col] = ppi_df[reg_ppi_col].apply(normalize_region)
...
ppi_df = (
    ppi_df[[reg_ppi_col, ppi_val_col]]
    .rename(columns={reg_ppi_col: "region", ppi_val_col: "PPI"})
)
...
if merged.empty:
    st.error("❌ 공통 지역이 없습니다. CSV의 지역명을 확인하세요.")
    st.stop()
...
if view == "지역별 표":
    ...
    males = [-row[c] for c in male_cols]
    females = [row[c] for c in female_cols]
    ages = [c.split("_")[-1] for c in male_cols]

    fig = go.Figure()
    fig.add_bar(y=ages, x=males, name="남자", orientation="h")
    fig.add_bar(y=ages, x=females, name="여자", orientation="h")
    fig.update_layout(
